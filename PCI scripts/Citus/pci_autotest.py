    ...
    try:
        ...
        if feature_category == "data_types":
            if feature_name == "Primitive Types":
                cursor.execute("CREATE TABLE test_primitive (id INT, name TEXT); SELECT create_distributed_table('test_primitive', 'id');")
            elif feature_name == "Complex Types":
                cursor.execute("CREATE TYPE test_complex AS (x INT, y TEXT);")
            elif feature_name == "JSONB":
                cursor.execute("CREATE TABLE test_jsonb (data JSONB); SELECT create_distributed_table('test_jsonb','data');")
            elif feature_name == "Geospatial Types":
                cursor.execute("CREATE EXTENSION postgis; CREATE TABLE test_geo (geom GEOMETRY); SELECT create_distributed_table('test_geo', 'geom');")
                ...
                cursor.execute("CREATE TABLE test_fts (content TSVECTOR); SELECT create_distributed_table('test_fts', 'content');")
            elif feature_name == "Vector":
                cursor.execute("CREATE EXTENSION vector; CREATE TABLE test_vector (embedding VECTOR(3)); SELECT create_distributed_table('test_vector', 'embedding');")
            ...
            if feature_name == "CTEs":
                ...
                cursor.execute("CREATE TABLE test_upsert (id INT PRIMARY KEY, data TEXT); SELECT create_distributed_table('test_upsert', 'id'); INSERT INTO test_upsert VALUES (1, 'test') ON CONFLICT (id) DO UPDATE SET data = 'updated';")
            ...
            if feature_name == "Stored Procedures":
                ...
                cursor.execute("SET LOCAL citus.multi_shard_modify_mode TO 'sequential'; CREATE TABLE test_trig (id INT); SELECT create_distributed_table('test_trig', 'id'); CREATE FUNCTION test_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN RETURN NEW; END; $$; CREATE TRIGGER trg BEFORE INSERT ON test_trig FOR EACH ROW EXECUTE FUNCTION test_trigger();")
            ...
            if feature_name == "Index Types":
                ...
                cursor.execute("""CREATE TABLE test_part (id INT) PARTITION BY RANGE (id); SELECT create_distributed_table('test_part', 'id');
                ...
                cursor.execute("""drop table if exists unlogged_pci_demo;
                                create unlogged table unlogged_pci_demo(n int primary key,flag char,text text); SELECT create_distributed_table('unlogged_pci_demo', 'n');"""
                )
            ...
            if feature_name == "Foreign Key":
                cursor.execute("CREATE TABLE parent (id INT PRIMARY KEY); SELECT create_distributed_table('parent', 'id'); CREATE TABLE child (parent_id INT , CONSTRAINT fk_customer FOREIGN KEY(parent_id) REFERENCES parent(id)); SELECT create_distributed_table('child', 'parent_id');")
            if feature_name == "Check":
                cursor.execute("CREATE TABLE test_check (id INT CHECK (id > 0)); SELECT create_distributed_table('test_check', 'id');")
            if feature_name == "Not Null":
                cursor.execute("CREATE TABLE test_notnull (id INT NOT NULL); SELECT create_distributed_table('test_notnull', 'id');")
            if feature_name == "Unique":
                cursor.execute("CREATE TABLE test_unique (id INT UNIQUE); SELECT create_distributed_table('test_unique', 'id');")
            #if feature_name == "DisableConstraint":
                #cursor.execute("alter table child disable trigger all;")
            if feature_name == "Exclusion":
                cursor.execute("CREATE EXTENSION IF NOT EXISTS btree_gist; CREATE TABLE test_exclusion (id int, t text, ts tstzrange, exclude using gist ((case when t ='A' THEN true end) with =,ts with && )); SELECT create_distributed_table('test_exclusion', 'id');")
            ...
            if feature_name == "Logical Replication":
                cursor.execute("CREATE TABLE test_replication (id INT PRIMARY KEY, value TEXT); SELECT create_distributed_table('test_replication', 'id');")
